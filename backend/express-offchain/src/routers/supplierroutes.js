...
router.post('/listproduct', async (req, res, next) => {
  ...
  try {
    ...
    const result = await supplier.listProduct(quantity, price, name, employeeAddress, address);
...
router.get('/viewselfproduct', async (req, res, next) => {
  ...
  try {
    const address = await getSupplierContractAddress(employeeAddress);
    const result = await supplier.viewSelfProduct(productId, employeeAddress, address);
...
router.get('/viewallselfproduct', async (req, res, next) => {
  ...
  try {
    ...
    const products = result.map(product => structParser.parseProduct(product));
